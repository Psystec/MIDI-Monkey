...
namespace MIDI_Monkey
{
    ...
    {
        private Dictionary<int, List<VirtualKey>> currentMappings = new Dictionary<int, List<VirtualKey>>();
        private string applicationName = string.Empty;
        private string currentFileName = string.Empty;
        private bool isNewFile = true;
        private bool hasUnsavedChanges = false;

        private readonly MidiKeyMapService _keyMapService = new MidiKeyMapService();
        ...
        private void SetupFormElements()
        {
            ...
            var keyValues = Enum.GetValues(typeof(VirtualKey))
                ...
                .Where(k => k != VirtualKey.LeftShift && k != VirtualKey.RightShift &&
                            k != VirtualKey.LeftAlt && k != VirtualKey.RightAlt &&
        ...
        private void DataGridViewKeyMappings_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                currentMappings[midiNote] = new List<VirtualKey>();

                if (modifier != "None" && Enum.TryParse(modifier, out VirtualKey modifierKey))
                {
                    currentMappings[midiNote].Add(modifierKey);
                }

                if (!string.IsNullOrEmpty(key) && Enum.TryParse(key, out VirtualKey virtualKey))
                {
        ...
        private void InitializeEmptyMappings()
        {
            ...
            for (int i = 0; i < 128; i++)
            {
                ...
                currentMappings[i] = new List<VirtualKey>();
            }
        }
        ...
        private async void SaveKeyMap()
        {
            try
            {
                ...
                var header = "# MIDI Monkey MidiKeyMap Config v2.0." + "\n" +
                    ...
                    "#   2. Modifier Key (Optional): The modifier key, if any (e.g., LeftShift, RightShift)" + "\n" +
                    "#   3. Alphanumeric Keyboard Key: The key on the keyboard (e.g., Q, W, Number1, Number0)" + "\n" +
        ...
        private async void buttonLoad_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                var keyMaps = await _keyMapService.GetAvailableKeyMapsAsync();
        ...
        private async void buttonRefresh_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                await _keyMapService.LoadKeyMapAsync(currentFileName);
